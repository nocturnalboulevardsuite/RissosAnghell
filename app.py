...
# --- CSS: SHADER DE CINE, CALLEJÓN ROJO Y ESCALERA TIPO GRINGA ---
css = """
<style>
...
.stApp {
    background-image: 
        linear-gradient(to bottom, rgba(30, 0, 0, 0.9), rgba(5, 0, 0, 0.98)),
...
/* --- ESCENOGRAFÍA: CALLEJÓN ROJO --- */
.alley-scene {
    position: relative;
    width: 100%;
    height: 450px;
    background: 
        linear-gradient(to bottom, rgba(120, 0, 0, 0.45), rgba(15, 0, 0, 0.95)),
...
/* --- ESCALERA TIPO GRINGA (FIRE ESCAPE) --- */
...
.fire-escape-ladder {
    ...
    transform: rotate(4deg) skewX(-2deg); /* Inclinación rota */
}
...
# Nota: El HTML dentro de st.markdown NO debe tener sangría (espacios al inicio), 
# de lo contrario Streamlit lo interpreta como un bloque de código markdown y lo renderiza como texto (el bug que tenías).
